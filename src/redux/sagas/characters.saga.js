...
function* fetchCharacterDetail(action) {
  try {
    const characterDetail = yield axios.get(
      `/api/characters/fetchcharacterdetails/${action.payload}`
    );
    yield put({
      type: 'SET_CHARACTER_DETAIL',
      payload: characterDetail.data[0],
    });
    const characterStatus = yield axios.get(
      `api/characters/fetchcharacterstatus/${action.payload}`
    );
    yield put({
      type: 'SET_CHARACTER_STATUS',
      payload: characterStatus.data[0],
    });
    const characterNotes = yield axios.get(
      `/api/characters/fetchCharacterNotes/${action.payload}`
    );
    yield put({ type: 'SET_CHARACTER_NOTES', payload: characterNotes.data });
    const characterContacts = yield axios.get(
      `/api/characters/fetchCharacterContacts/${action.payload}`
    );
    yield put({
      type: 'SET_CHARACTER_CONTACTS',
      payload: characterContacts.data,
    });
    const characterLifestyle = yield axios.get(
      `/api/characters/fetchCharacterLifestyle/${action.payload}`
    );
    yield put({
      type: 'SET_CHARACTER_LIFESTYLE',
      payload: characterLifestyle.data,
    });

    const characterArmor = yield axios.get(
      `api/characters/fetchcharacterarmor/${action.payload}`
    );
    const characterShield = yield axios.get(
      `api/characters/fetchcharactershield/${action.payload}`
    );
    const characterWeapons = yield axios.get(
      `api/characters/fetchcharacterweapons/${action.payload}`
    );
    const characterGrenades = yield axios.get(
      `api/characters/fetchcharactergrenades/${action.payload}`
    );
    const characterMiscGear = yield axios.get(
      `api/characters/fetchCharacterMiscGear/${action.payload}`
    );
    const characterPharmaGear = yield axios.get(
      `api/characters/fetchCharacterPharmaGear/${action.payload}`
    );
    const characterCyberBridgeDetail = yield axios.get(
      `/api/characters/fetchcharactercyberdetails/${action.payload}`
    );
    const characterNetrunningGear = yield axios.get(
      `api/characters/fetchcharacterNetrunningGear/${action.payload}`
    );
    const characterVehicles = yield axios.get(
      `api/characters/fetchcharacterVehicles/${action.payload}`
    );
    const characterActiveVehicleMods = yield axios.get(
      `api/characters/characterActiveVehicleMods/${action.payload}`
    );
    yield put({
...
function* changeCharacterHealth(action) {
  try {
    ...
    const characterStatus = yield axios.get(
      `api/characters/fetchcharacterstatusbystatusid/${action.payload.charStatusID}`
    );
    yield put({
      type: 'SET_CHARACTER_STATUS',
      payload: characterStatus.data[0],
...
function* useConsumableFromPack(action) {
  try {
    yield axios.delete(
      `/api/characters/useConsumable/${action.payload.foodstuff.char_gear_bridge_id}`
    );
    const characterMiscGear = yield axios.get(
      `api/characters/fetchCharacterMiscGear/${action.payload.charID}`
    );
    yield put({
      type: 'SET_CHARACTER_MISC_GEAR',
      payload: characterMiscGear.data,
    });
    yield put({ type: 'SET_CHARSHEET_LOAD_STATUS', payload: false });
  } catch (err) {
...
function* usePharmaceuticalFromPack(action) {
  try {
    yield axios.delete(
      `/api/characters/usePharmaceutical/${action.payload.pharma.char_pharma_bridge_id}`
    );
    const characterPharmaGear = yield axios.get(
      `api/characters/fetchCharacterPharmaGear/${action.payload.charID}`
    );
    yield put({
      type: 'SET_CHARACTER_PHARMA_GEAR',
      payload: characterPharmaGear.data,
    });
    yield put({ type: 'SET_CHARSHEET_LOAD_STATUS', payload: false });
  } catch (err) {
...
function* charUseGrenade(action) {
  yield axios.delete(
    `/api/characters/useGrenade/${action.payload.grenade.grenade_bridge_id}`
  );
  // fetch / set grenades
  const characterGrenades = yield axios.get(
    `api/characters/fetchcharactergrenades/${action.payload.charID}`
  );
  yield put({
    type: 'SET_CHARACTER_GRENADES',
    payload: characterGrenades.data,
  });
  yield put({ type: 'SET_CHARSHEET_LOAD_STATUS', payload: false });
}
...
function* arbitraryBankChange(action) {
  try {
    yield axios.put(
      `/api/characters/savecharacterbank/${action.payload.charID}`,
      action.payload
    );
    const bank = yield axios.get(
      `/api/characters/fetchBank/${action.payload.charID}`
    );
    yield put({ type: 'SET_CHARACTER_BANK', payload: bank.data });
    yield put({ type: 'SET_CHARSHEET_LOAD_STATUS', payload: false });
  } catch (err) {
...
function* characterCreatePharmaceutical(action) {
  try {
    yield axios.put(
      'api/characters/charactercreatepharmaceutical/',
      action.payload
    );
    const advancementGear = yield axios.get(
      `/api/characters/fetchAdvancementGear/${action.payload.characterID}`
    );
    yield put({
      type: 'SET_ADVANCEMENT_MISC_GEAR',
      payload: advancementGear.data,
    });
    const characterMiscGear = yield axios.get(
      `api/characters/fetchCharacterMiscGear/${action.payload.characterID}`
    );
    yield put({
      type: 'SET_CHARACTER_MISC_GEAR',
      payload: characterMiscGear.data,
...
function* createCharacterNote(action) {
  try {
    ...
    const characterNotes = yield axios.get(
      `/api/characters/fetchCharacterNotes/${action.payload.charID}`
    );
    yield put({ type: 'SET_CHARACTER_NOTES', payload: characterNotes.data });
    yield put({ type: 'SET_CHARSHEET_LOAD_STATUS', payload: false });
  } catch (error) {
...
function* deleteCharacterNote(action) {
  try {
    yield axios.delete(
      `api/characters/deleteCharacterNote/${action.payload.noteID}`
    );
    const characterNotes = yield axios.get(
      `/api/characters/fetchCharacterNotes/${action.payload.charID}`
    );
...
function* createCharacterContact(action) {
  try {
    ...
    const characterContacts = yield axios.get(
      `/api/characters/fetchCharacterContacts/${action.payload.char_id}`
    );
    yield put({
      type: 'SET_CHARACTER_CONTACTS',
      payload: characterContacts.data,
...
function* updateCharacterContact(action) {
  try {
    ...
    const characterContacts = yield axios.get(
      `/api/characters/fetchCharacterContacts/${action.payload.charID}`
    );
    yield put({
      type: 'SET_CHARACTER_CONTACTS',
      payload: characterContacts.data,
...
function* deleteCharacterContact(action) {
  try {
    yield axios.delete(
      `api/characters/deleteCharacterContact/${action.payload}`
    );
  } catch (error) {
...
function* saveGameMasterCharacter(action) {
  try {
    yield axios.put(
      `api/characters/savegamemastercharacter/${action.payload.charDetail.id}`,
      action.payload
    );
  } catch (error) {
...
function* deleteGameMasterCharacter(action) {
  try {
    yield axios.delete(
      `api/characters/deletegamemastercharacter/${action.payload.charDetailID}`
    );
  } catch (error) {
...
function* saveGameMasterContact(action) {
  try {
    yield axios.put(
      `api/characters/savegamemastercontact/${action.payload.campaign_id}`,
      action.payload
    );
...
function* deleteGameMasterContact(action) {
  try {
    yield axios.delete(
      `/api/characters/deletegamemastercontact/${action.payload}`
    );
    yield put({ type: 'FETCH_GM_CONTACTS' });
  } catch (error) {
...
function* fetchAdvancementDetails(action) {
  try {
    const advancementDetails = yield axios.get(
      `/api/characters/fetchAdvancementDetails/${action.payload}`
    );
    yield put({
    ...
    const characterStatus = yield axios.get(
      `api/characters/fetchAdvancementStatus/${action.payload}`
    );
    yield put({
    ...
    const advancementArmor = yield axios.get(
      `/api/characters/fetchAdvancementArmor/${action.payload}`
    );
    const advancementShield = yield axios.get(
      `/api/characters/fetchAdvancementShield/${action.payload}`
    );
    const advancementWeapons = yield axios.get(
      `/api/characters/fetchAdvancementWeapons/${action.payload}`
    );
    const advancementGrenades = yield axios.get(
      `/api/characters/fetchAdvancementGrenades/${action.payload}`
    );
    const advancementGear = yield axios.get(
      `/api/characters/fetchAdvancementGear/${action.payload}`
    );
    const advancementPharma = yield axios.get(
      `/api/characters/fetchAdvancementPharma/${action.payload}`
    );
    const advancementCyber = yield axios.get(
      `/api/characters/fetchAdvancementCyber/${action.payload}`
    );
    const advancementCyberSlots = yield axios.get(
      `/api/characters/fetchAdvancementCyberSlots/${action.payload}`
    );
    const advancementNetrunnerGear = yield axios.get(
      `/api/characters/fetchNetrunnerGear/${action.payload}`
    );
    const advancementVehicles = yield axios.get(
      `/api/characters/fetchAdvancementVehicle/${action.payload}`
    );
    const advancementVehicleMods = yield axios.get(
      `/api/characters/fetchAdvancementVehicleMods/${action.payload}`
    );
    const advancementClothes = yield axios.get(
      `/api/characters/fetchAdvancementClothes/${action.payload}`
    );

    yield put({
      type: 'SET_ADVANCEMENT_GEAR',
      payload: {
    ...
    const advancementActiveVehicleMods = yield axios.get(
      `/api/characters/fetchAdvancementActiveVehicleMods/${action.payload}`
    );
    yield put({
      type: 'SET_ONE_CHARACTER_VEHICLE_MODS',
...
function* fetchAdvancementClothes(action) {
  ...
  yield put({
    type: 'SET_ADVANCEMENT_CLOTHES',
    payload: advancementClothes.data,
  });
}
...
function* fetchAdvancementShield(action) {
  ...
  yield put({
    type: 'SET_ADVANCEMENT_SHIELD',
    payload: advancementShield.data,
  });
}
...
function* fetchAdvancementWeapons(action) {
  ...
  yield put({
    type: 'SET_ADVANCEMENT_WEAPONS',
    payload: advancementWeapons.data,
  });
}
...
function* fetchAdvancementGrenades(action) {
  ...
  yield put({
    type: 'SET_ADVANCEMENT_GRENADES',
    payload: advancementGrenades.data,
  });
}

function* fetchAdvancementMiscGear(action) {
  const advancementGear = yield axios.get(
    `/api/characters/fetchAdvancementMiscGear/${action.payload}`
  );
  yield put({
    type: 'SET_ADVANCEMENT_MISC_GEAR',
    payload: advancementGear.data,
...
function* fetchAdvancementPharma(action) {
  ...
  yield put({
    type: 'SET_ADVANCEMENT_PHARMA',
    payload: advancementPharma.data,
  });
}
...
function* fetchAdvancementCyberware(action) {
  ...
  yield put({
    type: 'SET_ADVANCEMENT_CYBERWARE',
    payload: advancementCyber.data,
  });
}
...
function* saveAdvancementDetails(action) {
  try {
    yield axios.put(
      `api/characters/saveAdvancementCharacter/${action.payload.char.id}`,
      action.payload
    );
...
function* fetchGameMasterContacts() {
  try {
    const allContacts = yield axios.get(
      '/api/characters/fetchgamemastercontacts'
    );
    yield put({ type: 'SET_CONTACT_LIST', payload: allContacts.data });
    const contactBridgeData = yield axios.get(
      '/api/characters/fetchgamemastercontactbridgedata'
    );
    yield put({
      type: 'SET_CONTACT_BRIDGE_DATA',
      payload: contactBridgeData.data,
    });
  } catch (error) {
...
function* characterBurnLuck(action) {
  try {
    axios.put(
      `/api/characters/characterBurnOneLuck/${action.payload.charID}`,
      action.payload
    );
  } catch (error) {
...
function* characterSaga() {
  ...
  yield takeLatest(
    'FETCH_ADVANCEMENT_VEHICLE_MODS',
    fetchAdvancementVehicleMods
  );
  ...
  yield takeLatest(
    'FETCH_GM_SINGLE_CHAR_CONTACTS',
    fetchGameMasterSingleCharContacts
  );
