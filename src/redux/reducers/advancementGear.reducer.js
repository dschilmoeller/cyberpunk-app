  ...
  switch (action.type) {
    ...
    case 'SET_ADVANCEMENT_DETAIL':
      return {
        ...
        totalCyberwareArmorQuality:
          action.payload.current_cyberware_armor_quality,
        totalCyberwareHealthBoxesCreated:
          action.payload.current_cyberware_health_boxes,
      };
    ...
    case 'SELL_ADVANCEMENT_NETRUNNER_GEAR':
      return {
        ...state,
        boughtNetrunnerGear: state.boughtNetrunnerGear.filter(
          (gear) => gear.netrunnerGearID !== action.payload.netrunnerGearID
        ),
      };
    case 'SELL_OWNED_NETRUNNER_GEAR':
      return {
        ...state,
        netrunnerGear: state.netrunnerGear.filter(
          (gear) =>
            gear.netrunner_bridge_id !== action.payload.netrunner_bridge_id
        ),
    ...
    case 'GM_REMOVE_GM_ARMOR':
      return {
        ...state,
        boughtArmor: state.boughtArmor.filter(
          (armor) => armor.armorID !== action.payload.armorID
        ),
      };
    case 'GM_REMOVE_ARMOR':
      return {
        ...state,
        armor: state.armor.filter(
          (armor) => armor.armor_bridge_id !== action.payload.armor_bridge_id
        ),
        soldArmor: [...state.soldArmor, action.payload],
        totalArmorQuality: action.payload.equipped
          ? state.totalArmorQuality - action.payload.quality
          : state.totalArmorQuality,
      };
    case 'GM_GIVE_SHIELD':
      return {
        ...state,
    ...
    case 'GM_REMOVE_GM_SHIELD':
      return {
        ...state,
        boughtShield: state.boughtShield.filter(
          (shield) => shield.shieldID !== action.payload.shieldID
        ),
      };
    case 'GM_REMOVE_SHIELD':
      return {
        ...state,
        shield: state.shield.filter(
          (shield) =>
            shield.shield_bridge_id !== action.payload.shield_bridge_id
        ),
        soldShield: [...state.soldShield, action.payload],
        totalShieldQuality: action.payload.equipped
          ? state.totalShieldQuality - action.payload.quality
          : state.totalShieldQuality,
      };
    case 'GM_GIVE_WEAPON':
      return {
        ...state,
    ...
    case 'GM_REMOVE_GM_WEAPON':
      return {
        ...state,
        boughtWeapons: state.boughtWeapons.filter(
          (weapon) => weapon.weaponID !== action.payload.weaponID
        ),
      };
    case 'GM_REMOVE_WEAPON':
      return {
        ...
        weapons: state.weapons.filter(
          (weapon) =>
            weapon.weapon_bridge_id !== action.payload.weapon_bridge_id
        ),
    ...
    case 'GM_REMOVE_GM_GRENADE':
      return {
        ...state,
        boughtGrenades: state.boughtGrenades.filter(
          (grenade) => grenade.grenadeID !== action.payload.grenadeID
        ),
      };
    case 'GM_REMOVE_GRENADE':
      return {
        ...state,
        grenades: state.grenades.filter(
          (grenade) =>
            grenade.grenade_bridge_id !== action.payload.grenade_bridge_id
        ),
    ...
    case 'GM_REMOVE_GM_MISC_GEAR':
      return {
        ...state,
        boughtMiscGear: state.boughtMiscGear.filter(
          (gear) => gear.miscGearID !== action.payload.miscGearID
        ),
      };
    case 'GM_REMOVE_MISC_GEAR':
      return {
        ...state,
        gear: state.gear.filter(
          (gear) =>
            gear.char_gear_bridge_id !== action.payload.char_gear_bridge_id
        ),
    ...
    case 'GM_REMOVE_GM_CYBERWARE':
      return {
        ...state,
        boughtCyberware: state.boughtCyberware.filter(
          (cyberware) => cyberware.cyberwareID !== action.payload.cyberwareID
        ),
      };
    case 'GM_REMOVE_CYBERWARE':
      return {
        ...state,
        cyberware: state.cyberware.filter(
          (cyberware) =>
            cyberware.owned_cyberware_id !== action.payload.owned_cyberware_id
        ),
    ...
    case 'GM_UNEQUIP_CYBERWARE':
      return {
        ...
        cyberware: state.cyberware.map((item) => {
          if (
            item.owned_cyberware_id ===
            action.payload.incomingCyber.owned_cyberware_id
          ) {
            item.equipped = false;
            return item;
          }
    ...
    case 'GM_REMOVE_GM_VEHICLE':
      return {
        ...state,
        boughtVehicles: state.boughtVehicles.filter(
          (gear) => gear.vehicleID !== action.payload.vehicleID
        ),
      };
    case 'GM_REMOVE_VEHICLE':
      return {
        ...
        vehicles: state.vehicles.filter(
          (gear) => gear.vehicle_bridge_id !== action.payload.vehicle_bridge_id
        ),
    ...
    case 'GM_GIVE_VEHICLE_MOD':
      return {
        ...state,
        boughtVehicleMods: [
          ...state.boughtVehicleMods,
          {
            char_id: action.payload.item.char_id,
            char_owned_vehicle_mods_id:
              action.payload.item.char_owned_vehicle_mods_id,
            description: action.payload.item.description,
    ...
    case 'GM_REMOVE_GM_VEHICLE_MOD':
      return {
        ...state,
        boughtVehicleMods: state.boughtVehicleMods.filter(
          (mod) => mod.vehicleModID !== action.payload.vehicleModID
        ),
      };
    case 'GM_REMOVE_VEHICLE_MOD':
      return {
        ...state,
        vehicleMods: state.vehicleMods.filter(
          (mod) =>
            mod.char_owned_vehicle_mods_id !==
            action.payload.char_owned_vehicle_mods_id
        ),
