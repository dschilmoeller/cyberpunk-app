...
function CreationAttributes() {
  ...
  const [attributeNumber, setAttributeNumber] = useState(
    charDetail.attributeNumber
  );
  const [attributeCounter, setAttributeCounter] = useState(
    charDetail.attributeCounter
  );

  const [attributeArray, setAttributeArray] = useState([]);

  const [attributeSelectionHistory, setAttributeSelectionHistory] = useState(
    charDetail.attributeSelectionHistory
  );
  ...
  const AttributeSelector = (attribute) => {
    dispatch({
      type: 'CREATION_SELECT_ATT',
      payload: { att: attribute.toLowerCase(), value: attributeNumber },
    });
    increaseCounter();
  };
  ...
  const undoLastSelection = () => {
    if (attributeSelectionHistory.length < 1) {
      ...
      dispatch({
        type: 'CREATION_UNDO_SELECT_ATT',
        payload: attributeSelectionHistory.pop(),
      });
      decreaseCounter();
    }
  };
  ...
  return (
    <>
      ...
      <Grid container display={'flex'} justifyContent={'center'} spacing={1}>
        ...
        <Grid item xs={12}>
          <Item sx={{ height: 1 }}>
            Attributes are the fundamental traits of your character - almost all
            rolls in Cyberpumpkin use one Attribute + a skill or other trait.
          </Item>
        </Grid>
        <Grid item xs={12}>
          <Item sx={{ height: 1 }}>
            Selections: One attribute is your primary, and is selected at 4 -
            this is your most outstanding attribute, far beyond the average
            person. Three additional attributes are selected at 3; these are
            your characters other above average traits. Three additional
            attributes are selected at 2 - this is about average for most humans
            - and finally one attribute is selected at 1 - you can't excel at
            everything, unfortunately.
          </Item>
        </Grid>
        <Grid item xs={12}>
          <Item sx={{ height: 1 }}>
            Special Attributes: All characters start with 1 Street Cred.
          </Item>
        </Grid>
        <Grid item xs={12}>
          <Item sx={{ height: 1 }}>
            Click an attribute name to learn more about it, including examples
            of what different ranks might look like!
          </Item>
        </Grid>
      </Grid>
      ...
      <Grid container>
        <Grid item xs={12} textAlign={'center'}>
          <Button
            sx={{ margin: 1 }}
            variant="contained"
            onClick={() => resetAttributeSelection()}
          >
            ...
            <React.Fragment key={stat.attName}>
              <Grid item xs={4}>
                <Grid container>
                  <Grid xs={4} item>
                    <Item>
                      <AttributesDialog prop={stat.attName} />
                    </Item>
                  </Grid>
                  {stat.value === 0 ? (
                    <Grid xs={8} item>
                      <Item
                        sx={{ cursor: 'pointer' }}
                        onClick={() => AttributeSelector(stat.attName)}
                      >
                        Select at {attributeNumber} points
                      </Item>
                    </Grid>
                  ) : (
                    <>
                      ...
                      <Grid xs={8} item>
                        <Item>{dotReturn(stat.value)}</Item>
                      </Grid>
                    </>
                  )}
                </Grid>
