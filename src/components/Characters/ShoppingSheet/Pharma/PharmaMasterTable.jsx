...
export default function PharmaMasterTable() {
  ...
  const buyMiscGear = (item) => {
    if (charDetail.bank >= item.price) {
      ...
      dispatch({
        type: 'BUY_ITEM',
        payload: {
          itemMasterID: item.pharma_master_id,
          newBank,
          charID: charDetail.id,
          table: 'char_pharma_bridge',
          column: 'pharma_master_id',
        },
      });
    } else {
  ...
  for (let i = 0; i < pharmaMaster.length; i++) {
    otherMasterRows.push(
      createOtherMasterData(
        pharmaMaster[i].description,
        pharmaMaster[i].pharma_master_id,
        pharmaMaster[i].name,
        pharmaMaster[i].price
      )
    );
  }
