...
export default function VehicleOwnedTable() {
  ...
  const nomadFreebieStatus = useSelector(
  ...
  const sellOwnedVehicle = (item) => {
    let newBank = Number(
      charDetail.bank + Math.floor((item.price + item.mod_total_cost) / 4)
    );
    dispatch({
      type: 'SELL_ITEM',
      payload: {
        itemID: item.vehicle_bridge_id,
        newBank,
        charID: charDetail.id,
        table: 'char_vehicle_bridge',
        column: 'vehicle_bridge_id',
      },
    });
  };
                      ...
                      return (
                        <TableRow hover key={row.vehicle_bridge_id}>
                          ...
                          <TableCell align="center">
                            {euroBuck}
                            {Math.floor(
                              (row.price + row.total_mod_cost) / 4
