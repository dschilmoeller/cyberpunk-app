...
export default function CharacterVehicles() {
  ...
  const charVehicleMods = useSelector(
    (store) => store.characterGear.vehicleMods
  );
  ...
  const healthBuilder = (maxHealth, currentDamage, incomingKey) => {
    ...
    for (let i = 0; i < currentDamage; i++) {
      healthArray.push(
        <Grid item key={i} xs={1.2}>
          <Item onClick={(e) => handleHealthDamage(e, incomingKey)}>
            {aggMarker}
          </Item>
        </Grid>
      );
    ...
    for (let i = 0; i < current_armor_damage; i++) {
      armorArray.push(
        <Grid key={i} item xs={1.2}>
          <Item onClick={(e) => handleArmorDamage(e, incomingKey)}>
            {aggMarker}
          </Item>
        </Grid>
      );
  ...
  const handleArmorDamage = (e, incomingKey) => {
    if (e.target.innerText === unhurtMarker) {
      ...
      dispatch({
        type: 'VEHICLE_REMOVE_ONE_ARMOR_DAMAGE',
        payload: incomingKey,
      });
    }
  };
    ...
    for (let i = 0; i < shotCaller; i++) {
      bulletArray.push(
        <Grid item key={i} xs={xsNumber}>
          <Item onClick={(e) => handleVehicleWeaponShot(e)}>
            {unhurtMarker}
          </Item>
        </Grid>
      );
        ...
        {charVehicles.map((item) => {
          return (
            <Fragment key={item.vehicle_bridge_id}>
              ...
              <Grid item xs={4}>
                <Item>Move: {item.move}</Item>
              </Grid>
              <Grid item xs={4}>
                <Item>Top Speed (mph): {item.mph}</Item>
              </Grid>
              <Grid item xs={4}>
                <Item>Seats: {item.seats + item.extra_seats}</Item>
              </Grid>
              ...
              {charVehicleMods.map((mod) => {
                if (
                  ...
                  mod.vehicle_bridge_id === item.vehicle_bridge_id
                ) {
                  return (
                  ...
                  return (
                    <Fragment key={mod.char_vehicle_mod_bridge_id}>
                      ...
                      <Grid item xs={4}>
                        <Item>DV: {item.type === 'Bike' ? '8' : '11'}</Item>
                      </Grid>
                      <Grid item xs={4}>
                        <Item>ROF: 1</Item>
                      </Grid>
