...
function Weapons() {
    ...
    const ClipButtonBuilder = (damageType, incomingKey) => {
        switch (damageType) {
            case 'melee':
        ...
        for (let i = 0; i < charWeapons.length; i++) {
            weapons.push(
                <React.Fragment key={i}>
                    ...
                    {ClipButtonBuilder(charWeapons[i].dmg_type, i)}
                    {clipBuilder(charWeapons[i].max_clip, charWeapons[i].current_shots_fired, i)}
                </React.Fragment>
            )
        }
        return weapons
