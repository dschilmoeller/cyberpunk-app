...
export default function CharacterNetrunner() {
    ...
    const rezBoxBuilder = (rez, current_rez_damage, netrunner_bridge_id) => {
        ...
        for (let i = 0; i < rez; i++) {
            if (i + 1 > current_rez_damage) {
                rezArray.push(<Grid key={i} item xs={2} onClick={(e) => rezBoxChanger(e, netrunner_bridge_id, rez, current_rez_damage)}><Item>{unhurtMarker}</Item></Grid>)
            } else {
                rezArray.push(<Grid key={i} item xs={2} onClick={(e) => rezBoxChanger(e, netrunner_bridge_id)}><Item>{aggMarker}</Item></Grid>)
            }
        }
        return rezArray
    }
    ...
    const rezBoxChanger = (e, netrunner_bridge_id, rez, current_rez_damage) => {
        if (e.target.innerText === unhurtMarker) {
            if (current_rez_damage + 1 === rez) {
                dispatch({ type: 'NETRUNNER_SOFTWARE_DEACTIVATED', payload: netrunner_bridge_id })
            } else {
    ...
    const activeBoxBuilder = (netrunner_bridge_id, active) => {
        if (active === true) {
            return <Grid item xs={2} onClick={(e) => activeBoxChanger(e, netrunner_bridge_id)}><Item>{aggMarker}</Item></Grid>
        } else {
            return <Grid item xs={2} onClick={(e) => activeBoxChanger(e, netrunner_bridge_id)}><Item>{unhurtMarker}</Item></Grid>
        }
    }
    ...
    const netrunnerCalculate = () => {
        ...
        charNetrunnerGear.map(gear => {
            if (gear.equipped === true && gear.type === 'software' && gear.attack > highAttack && gear.active === true) {
        ...
        let highDefense = 0
        charNetrunnerGear.map(gear => {
            if (gear.equipped === true && gear.type === 'software' && gear.defense > highDefense && gear.active === true) {
        ...
        charNetrunnerGear.map(gear => {
            if (gear.equipped === true && gear.type === 'software' && gear.name === 'Armor' && gear.active === true) {
                soak += 1
            }
        })
    ...
    return (<>
        <Grid container spacing={2}>
                        ...
                        <Fragment key={gear.netrunner_bridge_id}>
                            <Grid item xs={12}>
                                <Grid container paddingBottom={2}>
                                    ...
                                    <Grid item paddingLeft={1} paddingRight={1} xs={4}><Item>Software Activations per Netrunner Action: {activationsPerAction}</Item></Grid>
                                </Grid>
                                <Grid container paddingBottom={2}>
                                    <Grid item paddingLeft={1} paddingRight={1} xs={4}><Item>Net Attack: {netrunnerAttack}</Item></Grid>
                                    <Grid item paddingLeft={1} paddingRight={1} xs={4}><Item>Net Defense: {netrunnerDefense}</Item></Grid>
                                    <Grid item paddingLeft={1} paddingRight={1} xs={4}><Item>Net Soak: {netrunnerSoak}</Item></Grid>
                                </Grid>
                            </Grid>
                        </Fragment>
                    )
            ...
            <Grid container paddingTop={2} spacing={1}>
                <Grid item xs={12}><Item>Programs - Click REZ to reload (2 NET actions)</Item></Grid>
            </Grid>
                        ...
                        <Fragment key={gear.netrunner_bridge_id}>
                            <Grid item xs={6} >
                                <Grid container>
                                    ...
                                    <Grid item xs={2}><Item>Attack</Item></Grid>
                                    <Grid item xs={2}><Item>{gear.attack}</Item></Grid>
                                    <Grid item xs={2}><Item>Defense</Item></Grid>
                                    <Grid item xs={2}><Item>{gear.defense}</Item></Grid>
                                    <Grid item xs={2}><Item>Active?</Item></Grid>
                                    {activeBoxBuilder(gear.netrunner_bridge_id, gear.active)}

                                    <Grid item xs={2}>
                                        <Item><Button onClick={() => reloadSoftware(gear.netrunner_bridge_id)}>REZ</Button></Item>
                                    </Grid>
                                    {rezBoxBuilder(gear.rez, gear.current_rez_damage, gear.netrunner_bridge_id)}
