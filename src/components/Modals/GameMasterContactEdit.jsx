...
export default function GameMasterContactEdit({ prop }) {
  ...
  const [descText, setDescText] = React.useState(
    prop == 'new' ? '' : prop.description
  );
  const [connectionAmount, setConnectionAmount] = React.useState(
    prop == 'new' ? 0 : prop.connection
  );

  const [campaign, setCampaign] = React.useState(
    prop == 'new' ? 0 : prop.campaign_id
  );
    ...
    if (nameText.length > 0 && descText.length > 0 && connectionAmount > 0) {
      if (campaign != 0) {
        if (prop === 'new') {
          ...
          dispatch({
            type: 'SAVE_GM_CONTACT',
            payload: {
              contact_master_id: prop.contact_master_id,
              name: nameText,
              connection: connectionAmount,
              description: descText,
              campaign_id: campaign,
      ...
      characterList.map((character) => {
        if (character.campaign === campaign) {
          ...
          for (let i = 0; i < contactBridge.length; i++) {
            if (
              character.id === contactBridge[i].char_id &&
              prop.contact_master_id === contactBridge[i].contact_id
            ) {
              contactExists = true;
              return;
            }
          ...
          if (contactExists === false) {
            dispatch({
              type: 'ASSIGN_CONTACT_CAMPAIGN_CHARS',
              payload: {
                contactID: prop.contact_master_id,
  ...
  const assignContactSingleChar = (character) => {
    if (character == 0) {
      ...
      for (let i = 0; i < contactBridge.length; i++) {
        if (
          character === contactBridge[i].char_id &&
          prop.contact_master_id === contactBridge[i].contact_id
        ) {
  ...
  return (
    <>
      ...
      <Button onClick={handleClickOpen('paper')} variant="contained">
        {prop == 'new' ? 'New Contact' : 'Edit Contact'}
      </Button>
      <Dialog
        open={open}
      ...
      >
        {prop == 'new' ? (
          <DialogTitle id="scroll-dialog-title">New Contact</DialogTitle>
        ) : (
              ...
              >
                {
                  <MenuItem key={0} value={0}>
                    Select Campaign to assign Contact into
                  </MenuItem>
                }
                {campaignList.map((campaign) => {
                  return (
              ...
              <>
                ...
                <Grid item xs={6} padding={2}>
                  <Button
                    ...
                    variant="contained"
                    color="success"
                    onClick={() => assignContactAllChars(campaign)}
                  >
                    Assign to all Characters in campaign
                  </Button>
                </Grid>
                <Grid item xs={6} padding={2}>
                  <Select
                ...
                <Grid item xs={6} padding={2}>
                  <Button
                    ...
                    variant="contained"
                    color="success"
                    onClick={() => assignContactSingleChar(characterID)}
                  >
                    Assign to individual character
                  </Button>
                </Grid>
              </>
            ) : (
