...
export default function CharSheetWeaponDialog({ prop }) {
            ...
            case 'Heavy SMG':
                return (<>
                    <Grid container fontFamily={'serif'}>
                        ...
                        <Grid item padding={1} xs={12}>Characters firing a weapon in full automatic have a ROF of 1 and a base difficulty of 5 on their attack. Their weapon must have 10 or more rounds remaining in its clip. The attacker cannot perform aimed shots, and additional successes on the attack do not provide additional damage. However, the attacker can elect to remove one or more dice from their attack die pool. If they do so, they can force their opponent to remove the same number of dice from their Reflexes roll to dodge <b>OR</b> increase the amount of armor lost by the target by the number of dice removed, provided they hit the target at all. Removing additional armor does not work if the target's armor is <i>Hardened</i>.</Grid>
                        <Grid item padding={1} xs={12}>Automatic Fire is only usable at the weapons normal range. If used at point blank range, the attacker has a difficulty of 4 AND trades dice at a 1:2 ratio - every die removed counts as two for the above rule.</Grid>
            ...
            case 'Cybersnake':
                return (<>
                    <Grid container fontFamily={'serif'}>
                        ...
                        <Grid item padding={1} xs={12}>Cybersnakes are horrifying weapons, consisting of a metal tentacle that erupts out of a handy orifice. Most users opt to implant them in the abdomen and have them come out of their mouth - the head of the weapon is commonly compared to a razor-blade roto rooter. The tentacle is approximately 3 feet long. The Cybersnake deals damage as a Heavy Melee Weapon. If the user successfully grapples an enemy before attacking with this weapon, the damage it causes is aggravated.</Grid>
                    </Grid>
                </>)
            case 'Scratchers':
            case 'Rippers':
