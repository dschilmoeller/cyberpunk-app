...
// Quantity?
export default function GMOwnedGrenade({ charDetail, charGrenades }) {
  const euroBuck = `\u20AC$`;
  ...
  for (let i = 0; i < charGrenades.length; i++) {
    charGrenadeRows.push(
      createCharGrenadeData(
                ...
                {sortedCharGrenadeRows.map((row) => {
                  return (
                    <TableRow hover key={row.grenade_bridge_id}>
                      ...
                      <TableCell align="center">
                        <Button onClick={() => gmRemoveGrenade(row)}>Remove</Button>
                      </TableCell>
                    </TableRow>
                  );
                })}
