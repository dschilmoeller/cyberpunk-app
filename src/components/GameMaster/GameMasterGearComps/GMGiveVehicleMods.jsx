...
export default function GMGiveVehicleMods() {
  ...
  const vehicleModID = useSelector(
    (store) => store.advancementGear.vehicleModID
  );
  ...
  const buyVehicleMod = (item) => {
    dispatch({
      type: 'GM_GIVE_VEHICLE_MOD',
      payload: { item, vehicleModID, price: 0 },
    });
  };
  ...
  function createVehicleModMasterData(
    description,
    name,
    type,
    price,
  ...
  for (let i = 0; i < vehicleModMaster.length; i++) {
    vehicleModMasterRows.push(
      createVehicleModMasterData(
        vehicleModMaster[i].description,
        vehicleModMaster[i].name,
        vehicleModMaster[i].type,
        vehicleModMaster[i].price,
        vehicleModMaster[i].vehicle_mod_master_id
      )
    );
  }
                  ...
                  if (row.type != 'All')
                    return (
                      <TableRow hover key={row.vehicle_mod_master_id}>
                        ...
                        <TableCell align="center">
                          <Button onClick={() => buyVehicleMod(row)}>
                            Buy
                          </Button>
                        </TableCell>
                      </TableRow>
