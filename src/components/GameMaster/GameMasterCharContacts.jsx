        ...
        switch (changeType) {
            case 'reduce':
                if (currentAmount - 1 >= 0) {
                    dispatch({ type: "GM_REDUCE_LOYALTY", payload: { newAmount: (currentAmount - 1), contactID: contactID } })
                } else {
            ...
            case 'increase':
                if (currentAmount + 1 < 10) {
                    dispatch({ type: "GM_INCREASE_LOYALTY", payload: { newAmount: (currentAmount + 1), contactID: contactID }})
                } else {
                        ...
                        <React.Fragment key={contact.char_contact_id}>
                            <Grid item xs={5.75} marginLeft={2.5} marginBottom={1} padding={1} border={'solid gray'}>
                                <Grid container>
                                    ...
                                    <Grid item xs={12}>Loyalty: {contact.loyalty}</Grid>
                                    <Grid item xs={5} margin={1}>
                                        <Button fullWidth variant='contained' color='error' onClick={() => changeLoyalty(contact.loyalty, 'reduce', contact.char_contact_id)}>
                                            Reduce Loyalty
                                        </Button>
                                    </Grid>
                                    <Grid item xs={5} margin={1}>
                                        <Button fullWidth variant='contained' color='success' onClick={() => changeLoyalty(contact.loyalty, 'increase', contact.char_contact_id)}>
                                            Increase Loyalty
                                        </Button>
                                    </Grid>
                                </Grid>
